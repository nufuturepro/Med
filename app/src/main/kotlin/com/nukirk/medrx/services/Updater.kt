...
import com.nukirk.medrx.UpdaterActivity
...
object Updater {
            ...
            if (connection.responseCode == 200) {
                ...
                if (jsonArray.length() > 0) {
                    ...
                    for (i in 0 until assets.length()) {
                        ...
                        if (assetName.endsWith(".apk") && !assetName.contains(
                                "wear",
                                ignoreCase = true
                            )
                        ) {
    ...
    fun startDownload(context: Context, url: String, version: String) {
        try {
            val fileName = "MedRX_$version.apk"
    ...
    fun showUpdateNotification(context: Context, updateInfo: UpdateInfo) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle(context.getString(R.string.update_available_version, updateInfo.version))
    ...
    private fun isNewer(remote: String, current: String): Boolean {
        try {
            ...
            val length = max(remoteParts.size, currentParts.size)
